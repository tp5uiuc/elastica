...
#include <chrono>
#include <limits>
#include <iostream> // Not recommended, but whatever man.

namespace Tools {

  // Prefer high_resolution_clock, but only if it's steady...
  template <bool HighResIsSteady =
  ...
  inline double benchmark(uint64_t samples, uint64_t iterations, const Op &op) {
  ...
  inline std::ostream& operator<<(std::ostream& os, BenchmarkResult const & result) {
    os << "Walltime : " << result.wall_time << "\n";
  ...
  template <typename Op>
  inline BenchmarkResult benchmark(const Op &op,
    ...
    const double accuracy =
        1.0 + std::min(std::max(0.001, config.accuracy), 0.1);
    ...
    for (;;) {
      ...
      double next_iters =
          std::max(min_time / std::max(times[0] * kMinSamples, 1e-9),
                   iters_per_sample * 2.0);
      iters_per_sample = (uint64_t)(next_iters + 0.5);
    }

    // - Keep taking samples until we are accurate enough (even if we run over
    // min_time).
    // - If we are already accurate enough but have time remaining, keep taking
    // samples.
    // - No matter what, don't go over max_time; this is important, in case
    // we happen to get faster results for the first samples, then happen to
    // transition to throttled-down CPU state.
